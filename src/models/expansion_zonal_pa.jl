function investment_pa()
    ...
    @constraint(m, rho[j=1:size(D, 1), z=1:length(Z)], p[j,z] == sum(y[i,j,z]
        for i=1:length(I)) + sum(y_bar[g,j] for g=Gz[z]) -
        sum(D[j,n] for n in Nz[z]) + yre[j,z] + s[j,z])
    @constraint(m, [j=1:size(D, 1), z=1:length(Z)], p[j,z] == sum(r[j,n] for n in Nz[z]))
    @constraint(m, [j=1:size(D, 1)], sum(r[j,n] for n in 1:length(N)) == 0)
    @constraint(m, [j=1:size(D, 1), l=1:length(L)],
        f_tilde[j,l] == sum(PTDF[n,l]*r[j,n] for n=1:length(N)))
    @constraint(m, [i=1:length(I), j=1:size(D, 1), z=1:length(Z)], y[i,j,z] + yrv[i,j,z] <=
        x[i,z])
    ...
    @objective(m, Min, sum(DT[j]*(VOLL*s[j,z] +
        ...
        sum(I[i]*x[i,z] for i=1:length(I), z=1:length(Z)) +
...
function redispatch_pa(x_pa::Array{Float64, 2}, x_bar_pa::Array{Float64, 1})
    ...
    @variable(model_rd, 0 <= yre_rd[j=1:size(D, 1), n=1:length(N)] <= R[j,n])
    ...
    @variable(model_rd, Z_tilde[1:length(N)] >= 0)
    @variable(model_rd, z_tilde_rd[1:size(D, 1), 1:length(N)] >= 0)

    # declare constraints

    @constraint(model_rd, [i=1:length(I), z=1:length(Z)], sum(x_rd[i,n] for n in Nz[z]) == x_pa[i,z])
    @constraint(model_rd, [j=1:size(D, 1), n=1:length(N)], z_tilde_rd[j,n] <= Z_tilde[n])
    @constraint(model_rd, [i=1:length(I), j=1:size(D, 1), n=1:length(N)], y_rd[i,j,n] <= x_rd[i,n])
    @constraint(model_rd, [g=1:length(X_bar), j=1:size(D, 1)], y_bar_rd[g,j] <= x_bar_pa[g])
    @constraint(model_rd, [j=1:size(D, 1), n=1:length(N)], r_rd[j,n] ==  z_tilde_rd[j,n] +
        sum(y_rd[i,j,n] for i=1:length(I)) + sum(y_bar_rd[g,j] for g=Gn[n]) +
        s_rd[j,n] + yre_rd[j,n] - D[j,n])
    ...
    # declare objective
    @objective(model_rd, Min, I_tilde*sum(Z_tilde) + sum(DT[j]*(MC_tilde*z_tilde_rd[j,n] + VOLL*s_rd[j,n] +
